...
public class VegetableAdapter extends RecyclerView.Adapter<VegetableAdapter.ProductHolder> {

    private List<Vegetables> results;
...
*/
    ...
    public VegetableAdapter(Context context, List<Vegetables> results,ClickListener clickListener) {
    ...
    @Override
    public VegetableAdapter.ProductHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.vegetables_list, parent, false);
        VegetableAdapter.ProductHolder holder=new VegetableAdapter.ProductHolder(view,listener);
        return holder;
    }

    @Override
    public void onBindViewHolder(VegetableAdapter.ProductHolder holder, int position) {
        Vegetables jobData = results.get(position);
